package com.javaproject.daytwentyfour;
...
//        Main main = new Main();
    ...
    public void sortWithPersonName() {
        addressArray.sort(Comparator.comparing(UserInterface::getFirstName));
        System.out.println(addressArray);
    }

    public void sortByCityStateZip() {
        List<UserInterface> sortPersonByStateCityZip=addressArray.stream().sorted(Comparator.comparing(UserInterface::getState).thenComparing(UserInterface::getCity).thenComparing(UserInterface::getZipCode)).collect(Collectors.toList());
